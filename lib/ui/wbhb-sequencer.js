...
import {Scheduler, CONTROL_CODES as SCHEDULER_CONTROL_CODES} from '../scheduler/scheduler.js';
import {WBHBSequencerControl} from './wbhb-sequencer-control.js';
import {ControlItem} from '../sequencer/items/ControlItem.js';
...
export class WBHBSequencer extends HTMLElement {
  ...
  _updateCurrentBar(newValue) {
    this.shadowRoot.querySelectorAll('wbhb-sequencer-track').forEach((el) => {el.offset = new Length(this.currentBar, Length.units.bar).to(Length.units.beat, {timeSignature: Scheduler.GLOBAL.timeSignature}).value;});
  }
