...
export class WBHBSequencerTrack extends HTMLElement {
  ...
  getCellText(col, offset) {
    const barNum = Math.floor(Math.floor((col + offset) / this.zoomLevel) / GlobalScheduler.timeSignature.upper + 1);
    const beatNum = Math.floor(Math.floor((col + offset) / this.zoomLevel) % GlobalScheduler.timeSignature.upper + 1);
  ...
  #updateContentItem(cell, offset, options) {
    ...
    const visible = offset % this.zoomLevel === 0;
    
    const col = Math.floor(offset/this.zoomLevel);
