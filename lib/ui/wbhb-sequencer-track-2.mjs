...
import { Beat } from '../sequencer2/BeatwiseSequencer.mjs';
...
export class WBHBSequencerTrack extends HTMLElement {
  ...
  #renderContentItems(offset) {
    ...
    for (const beats of this.sequencer.createMessageGen(null, {substituteOpaque: true})) {
    ...
    this.rows = Math.ceil(height/gridHeight);
  }
  ...
  #updateContentItem(cell, offset, options) {
    ...
    const width = Math.floor(cell.message.length / clocksPerCell) || 1;
  ...
  #dragenter(e) {
    if (e.dataTransfer.types.includes(MIDI.Message.mediaType)) {
      e.preventDefault();
    } else {
      return;
    }
  ...
  async #drop(e) {
    ...
    let message = new MIDI.Message(
      JSON.parse(
        e.dataTransfer.getData(MIDI.Message.mediaType)
      )
    );
    
    const beatIndex = (col + this.offset) * this.zoomLevel;

    const beat = this.sequencer.get(beatIndex);

    if (!beat) {
      const beat = new Beat();
      beat.add(message);
      this.sequencer.set(beatIndex, beat);
    }
