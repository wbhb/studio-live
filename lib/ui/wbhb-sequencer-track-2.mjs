...
export class WBHBSequencerTrack extends HTMLElement {
  ...
  #renderContentItem(item, offset, options) {
  ...
  #renderContentItems(offset) {
    ...
    for (const beats of this.sequencer.createMessageGen()) {
      ...
      if (i >= offset) {
        beats.forEach((beat) => {
          const el = this.#renderContentItem(beat, Math.floor(i/12));
          if (el) {
  ...
  #updateContentItems(offset, options) {
    ...
    cells.forEach((cell) => {this.#updateContentItem(cell, offset, options)});
  }
  ...
  #updateColumns(newValue, oldValue) {
