...
export class WBHBSequencerTrack extends HTMLElement {
  ...
  _renderContentItem(item, offset) {
    
    const col = item.timings.offset.to(Length.units.beat, {timeSignature: GlobalScheduler.timeSignature}).value;
    const width = item.timings.length.to(Length.units.beat, {timeSignature: GlobalScheduler.timeSignature}).value;
  ...
  _renderContentItems(offset) {
    ...
    const items = this.items?.flat().map((id, index) => {
    ...
    if (newInt > oldInt) {
      ...
      Array.from(underlay.querySelectorAll('.cell'))
        .forEach((el) => {
          ...
          this._renderContentItems(oldInt);
        });
      ...
      Array.from(underlay.querySelectorAll('.cell'))
        .forEach((el) => {
          ...
          this._renderContentItems(newInt);
        });
    }
