...
import {GLOBAL as GlobalScheduler, Scheduler, CONTROL_CODES} from '../scheduler/scheduler.mjs';
...
export class WBHBSequencerTrack extends HTMLElement {
  ...
  setupListeners() {
    ...
    GlobalScheduler.addEventListener('item', (e) => {this.handleItem(e)});
  }
  ...
  _renderUnderlayCell(col) {
    ...
    cell.innerText = this.getCellText(col);
    
    return cell;
  }
  
  getCellText(col) {
    const barNum = Math.floor((col + this.offset) / GlobalScheduler.timeSignature.upper + 1);
    const beatNum = Math.floor((col + this.offset) % GlobalScheduler.timeSignature.upper + 1);
  ...
  _renderContentItem(item) {
    ...
    const col = item.timings.offset.to(Length.units.beat).value;
    const width = item.timings.length.to(Length.units.beat).value;
  ...
  _renderContentItems() {
    ...
    this.#items.slice()
      ...
      .forEach((item) => {
        ...
        const el = this._renderContentItem(item);
  ...
  _updateOffset(newValue, oldValue) {
    ...
    Array.from(underlay.querySelectorAll('.cell'))
      .forEach((el) => {
        ...
        el.innerText = this.getCellText(col);
      });
    ...
    const listener = (e) => {
      ...
      content.classList.remove('scrub-right');
