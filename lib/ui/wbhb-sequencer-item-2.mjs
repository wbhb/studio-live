...
export class WBHBSequencerItem extends HTMLElement {
  ...
  setupListeners() {
    this.shadowRoot.querySelectorAll('.drag-handle[data-timingshandle]')
      .forEach((el) => {
        ...
        el.addEventListener("touchstart", (e) => {this.#boundListeners.dragstart(e.touches[0]);}, {passive: true});
        el.addEventListener("drag", this.#boundListeners.drag);
        el.addEventListener("touchmove", (e) => {this.#boundListeners.drag(e.touches[0]);}, {passive: true});
  ...
  render() {
    ...
    root.querySelector('.info').textContent = this.message;
