...
export class WBHBSequencer extends HTMLElement {
  ...
  static ZOOM_LEVELS = [192, 144, 96, 48, 24, 12, 6, 3, 1];
  ...
  #updateCurrentBar(newValue) {
    this.shadowRoot.querySelectorAll('wbhb-sequencer-track').forEach((el) => {
      const barLength = Scheduler.GLOBAL.timeSignature.upper + new Length(this.currentBar, Length.units.bar).to(Length.units.beat, {timeSignature: Scheduler.GLOBAL.timeSignature}).value;
      el.offset = barLength / this.zoomLevel;
    });
  }
