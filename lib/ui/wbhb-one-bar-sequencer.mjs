import {OneBarSequencer} from '../sequencer2/OneBarSequencer.mjs';
...
export class WBHBOneBarSequencer extends HTMLElement {
  ...
  set sequencer(sequencer) {
    ...
    for (const [note, track] of this.sequencer) {
  ...
  #clicked(evt) {
    ...
    if (dotEl.classList.contains('on')) {
      ...
      this.sequencer.get(note)[index] = OneBarSequencer.states.off;
    } else if (dotEl.classList.contains('off')) {
      dotEl.classList.remove('off');
      this.sequencer.get(note)[index] = OneBarSequencer.states.blank;
    } else {
      dotEl.classList.add('on');
      this.sequencer.get(note)[index] = OneBarSequencer.states.on;
    }
  }
}
