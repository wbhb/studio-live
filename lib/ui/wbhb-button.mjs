...
export class WBHBButton extends HTMLElement {
  ...
  constructor() {
    ...
    this.#boundListeners.closeAll = this.closeAll.bind(this);
    this.#boundListeners.mutation = this.#mutation.bind(this);
  }
  ...
  connectedCallback() {
    ...
    this.#mutationObserver = new MutationObserver(this.#boundListeners.mutation);
  ...
  setupListeners() {
    ...
    childButtons.forEach((child) => {
      child.addEventListener('closeall', this.#boundListeners.closeAll);
    });
  ...
  set midiMessage(newValue) {
    if (!newValue instanceof MIDI.Message) {
      throw new TypeError(`message must be a MIDI.Message`);
    }
    this.#midiMessage = newValue;
  }
  ...
  #dragStart(e) {
    ...
    if (e instanceof DragEvent) {
      ...
      if (this.midiMessage) {
        e.dataTransfer.setData(MIDI.Message.mediaType, JSON.stringify(this.midiMessage));
      }
      ...
      switch(mutation.type) {
        case 'childList':
          ...
          for (const child of mutation?.addedNodes) {
            if (child instanceof WBHBButton) {
              child.addEventListener('closeall', this.#boundListeners.closeAll);
            }
          }
          break;
