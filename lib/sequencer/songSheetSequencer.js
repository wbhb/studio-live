...
export class SongSheetSequencer extends Sequencer {
  ...
  _setupEventListeners() {
    ...
    this._scheduler.addEventListener("song", this._startSong.bind(this));
  }
  ...
  set items(newValue) {
    ...
    this._scheduler.dispatchEvent(evt);
  }
  ...
  _nextBar() {
    ...
    this._items.forEach((item) => {
      // const maxBeat = this._currentBeat + this._scheduler.timeSignature.upper + 1;
  ...
  _parseTimings(items) {
    ...
    let currentBar = 1;
    let currentBeatsInBar = this._scheduler.timeSignature.upper;
    let currentBeat = 1;
