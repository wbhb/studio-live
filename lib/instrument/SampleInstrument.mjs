...
export class SampleInstrument extends MIDIInstrument {
  ...
  #sampleMessageHandlers = new Map([
    [
      MIDI.Note_On, async (message, time) => {
        const node = await this.voices[message.note].start(time, message.velocity / 127.0);
        node.connect(this.getNamedOutput('Main Out').node);
      }
    ],
    [
