...
export class Tempo extends EventTarget {
    ...
    constructor(options) {
        ...
        if (config.BPM && config.beatTime) {
            throw new Error("Tempo cannot be created with 'BPM' and beatTime");
        }
    ...
    set BPM(newValue) {
        ...
        this.beatTime = SecondsInMinute / newValue / (this.#timeSignature.lower / 4);
    }
    
    get BPM() {
        let bpm = (1 / this.#beatTime) * SecondsInMinute /  (this.#timeSignature.lower / 4);
