...
import {MIDIItem} from '../sequencer/items/MIDIItem.js';
...
export class Scheduler extends EventTarget {
  ...
  _scheduler() {
    ...
    // console.log(`PERIOD: ${this.audioContext.currentTime}`);
    this.#queue.forEach((item) => {
      ...
      if (item.resolved) {
        ...
        if (start < (this.audioContext.currentTime + this.#config.lookahead)) {
          ...
          // console.log(`\tSch\t${item.type}: ${item.timings.start.value}`);
          ...
          // console.log(`\tQue\t${item.type}: ${item.timings.start.value}`);
        }
      }
    });
