...
export class Scheduler extends EventTarget {
  ...
  start() {
    ...
    this.#nextMidiClock = this.audioContext.currentTime;
  ...
  _scheduler() {
    ...
    if (this.#nextMidiClock < this.#audioCtx.currentTime + this.#config.lookahead) {
      
      const clockItem = new MIDIItem({
        message: {
          command: MIDI.Timing_Clock
        }
      });
      
      clockItem.resolve(new Length(this.#nextMidiClock, Length.units.second), {tempo: this.tempo});
      
      this.scheduleItem(clockItem);
      
      this.#nextMidiClock += this.tempo.beatTime / MIDI.CLOCKS_PER_BEAT;
    }
