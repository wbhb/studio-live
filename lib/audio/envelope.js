...
export class EnvelopeNode extends GainNode {
  ...
  start() {
    // this.gain.value = 0;
    this.gain.exponentialRampToValueAtTime(1.0, GlobalScheduler.audioContext.currentTime + this.#attack);
    this.gain.exponentialRampToValueAtTime(this.#sustain, GlobalScheduler.audioContext.currentTime + this.#attack + this.#decay);
  }
  
  stop() {
    
    this.gain.cancelAndHoldAtTime(GlobalScheduler.audioContext.currentTime)
    this.gain.exponentialRampToValueAtTime(0.0001, GlobalScheduler.audioContext.currentTime + this.#release);
