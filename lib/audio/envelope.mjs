...
export class EnvelopeNode extends GainNode {
  ...
  start(time) {
    ...
    this.gain.exponentialRampToValueAtTime(1.0, startTime + this.#attack);
    this.gain.exponentialRampToValueAtTime(this.#sustain, startTime + this.#attack + this.#decay);
  }
  ...
  stop(time) {
    ...
    this.gain.cancelAndHoldAtTime(stopTime)
    this.gain.exponentialRampToValueAtTime(0.0001, stopTime + this.#release);
